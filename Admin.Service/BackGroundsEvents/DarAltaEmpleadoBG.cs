...
{
    public class DarAltaEmpleadoBG(IUnitofWork _unitOfWork, IAuthService _apiAuthService, IMapper _mapper)
    {
        public async Task BGExecute() 
        {
            var data = await _unitOfWork.BacklLogsRepository.GetAllAsync(x => x.CompletedAt == null && x.EventType == (int)EventsEnum.DarAltaEmpleado);
